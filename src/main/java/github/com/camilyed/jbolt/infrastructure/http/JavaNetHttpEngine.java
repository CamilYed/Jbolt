...
/**
 * Professional Java HTTP engine using {@link java.net.http.HttpClient}. Supports all HTTP methods,
 * headers, JSON body, gzip decoding, and timing. Fully compatible with JBolt DSL and integration
 * tests.
 */
final class JavaNetHttpEngine implements HttpEngine {
  ...
  public Result<HttpResponse> execute(final HttpRequest request) {
    try {
      ...
      return Result.success(new HttpResponse(rawResponse.statusCode(), body, headers, duration));
  ...
  private static BodyPublisher getBodyPublisher(final HttpRequest request) {
    return request.body().map(BodyPublishers::ofString).orElseGet(BodyPublishers::noBody);
  }

  private static void applyMethod(
      final Builder builder, final HttpMethod method, final BodyPublisher bodyPublisher) {
    switch (method) {
  ...
  private static String decodeBody(final java.net.http.HttpResponse<byte[]> rawResponse)
      throws IOException {
  ...
  private static Map<String, String> extractHeaders(
      final java.net.http.HttpResponse<byte[]> rawResponse) {
