...
class DomainExecutionTest {
  ...
  void httpRequestRequiresUrl() {
    // expect
    assertThatThrownBy(() -> aRequest().withUrl(null).build())
  ...
  void httpRequestRequiresMethod() {
    // expect
    assertThatThrownBy(() -> aRequest().withMethod(null).build())
        .isInstanceOf(NullPointerException.class)
  ...
  void http2xxResponsesAreSuccessful(int statusCode) {
    // when
    HttpResponse response = aResponse().withStatus(statusCode).build();
  ...
  void httpRequestRequiresHeaders() {
    // expect
    assertThatThrownBy(() -> aRequest().withHeaders(null).build())
        .isInstanceOf(NullPointerException.class)
  ...
  void httpRequestAllowsNullBody() {
    // when
    HttpRequest request = aRequest().withBody(null).build();
