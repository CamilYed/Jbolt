...
public final class ResultAssertion<T> extends AbstractAssert<ResultAssertion<T>, Result<T>> {
    ...
    public ResultAssertion<T> isSuccess() {
        ...
        if (!actual.isSuccess()) {
            if (actual instanceof Result.Failure<T> fail) {
                failWithMessage("Expected result to be Success but was Failure with error: <%s>", fail.error().getMessage());
            } else {
    ...
    public ResultAssertion<T> isFailure() {
        isNotNull();
        if (actual.isSuccess()) {
            failWithMessage("Expected result to be Failure but was Success with value: <%s>", ((Result.Success<T>) actual).value());
        }
        return this;
    }
